...
if ( ! class_exists( 'WC_Connect_Service_Settings_Store' ) ) {
	...
	class WC_Connect_Service_Settings_Store {
		...
		private function decorate_history_entry( $entry ) {
			$user_id = $entry[ 'user_id' ];

			$entry[ 'user_name' ] = $this->get_user_name( $user_id );
			$entry[ 'user_url' ] = get_edit_user_link( $user_id );

			return $entry;
		}

		public function update_label_order_history( $order_id, $label_ids, $entry_type ) {
			$time = time() * 1000;
			if ( ! is_array( $label_ids ) ) {
				$label_ids = array( $label_ids );
			}

			$history_data = $this->get_label_history( $order_id, false );
			...
			array_unshift( $history_data, $entry );

			update_post_meta( $order_id, 'wc_connect_label_history', json_encode( $history_data ) );

			return $this->decorate_history_entry( $entry );
		}

		public function get_label_history( $order_id, $decorated ) {
			$raw_history_data = get_post_meta( ( int ) $order_id, 'wc_connect_label_history', true );
			if ( $raw_history_data ) {
				$history_data = json_decode( $raw_history_data, true, WOOCOMMERCE_CONNECT_MAX_JSON_DECODE_DEPTH );
			} else {
				$history_data = array();
			}

			if ( $decorated ) {
				foreach ( $history_data as $key => $entry ) {
					$history_data[ $key ] = $this->decorate_history_entry( $entry );
				}
			}
