...
**Overridable classes are contracts too, including which internal methods get called.** Third-party code extends the global `WC_Connect_*` classes (the abstract `WC_REST_Connect_Base_Controller` exists to be subclassed) and overrides individual methods, so the public and protected methods of any non-`final` class are contracts: changing their signatures or removing them breaks subclasses. Adding a fast path or skip that avoids calling an overridable method silently disables those overrides even though no signature changed: the override simply stops running. When optimizing such a class, ensure overridable methods are still invoked on every code path, or treat the change as breaking.
