...
# Use the project ruleset (phpcs.xml.dist aggregator) so the pre-commit auto-fixer
# stays aligned with `composer check-all`; the shared excludes/config live there.
...
./vendor/bin/phpcbf . --standard=./phpcs.xml.dist --filter=GitStaged --extensions=php
