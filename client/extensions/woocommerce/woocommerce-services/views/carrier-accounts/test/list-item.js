...
describe( "Carrier Accounts List Item", () => {
	describe( "for a disconnected carrier", () => {
		...
		it( "renders the carrier's name", function () {
			const renderedCarrierName = wrapper.find(
		...
		it( "does not render the carrier's credentials", function () {
			...
			expect( renderedCarrierName ).to.have.lengthOf( 1 );
			expect( renderedCarrierName.text()).to.equal( "");
		});

		it( "renders a connect button", function () {
			const renderedCarrierActions = wrapper.find(
			...
			expect( connectButton.children().text()).to.equal( "Connect");
		});
	...
	describe( "for a connected carrier", () => {
		...
		describe( "when the disconnect button is clicked", () => {
			...
			it( "renders a dialog to confirm disconnection", () => {
				expect(
					props.setVisibilityDisconnectCarrierDialog.mock.calls
				).to.have.lengthOf( 1 );
				expect(
					props.setVisibilityDisconnectCarrierDialog.mock.calls[ 0 ]
				).to.deep.equal([ 10, "carrier", true ]);
			...
			it( "disconnects the carrier when the disconnect button is pressed", () => {
				...
				expect(
					props.disconnectCarrier.mock.calls
				).to.have.lengthOf( 1 );
				expect(
					props.disconnectCarrier.mock.calls[ 0 ]
				).to.deep.equal([ "carrier" ]);
			});
		});
	});
});
