...
exports.isEventuallyPresent = isEventuallyPresent;
exports.isEventuallyVisible = isEventuallyVisible;
exports.logDebugLog = logDebugLog;
exports.logHTML = logHTML;
exports.scrollIntoView = scrollIntoView;
exports.waitAndClick = waitAndClick;
exports.waitAndType = waitAndType;
...
async function waitForSelector(page, selector, options = {}) {
  ...
  try {
    ...
    const secondsPassed = (new Date() - startTime) / 1000;
    console.log(`Failed to locate an element by locator: ${selector}. Waited for: ${secondsPassed} sec. URL: ${page.url()}`);
    throw e;
  }
}
 ...
 */
async function waitAndType(page, selector, value, options = {
  visible: true
}) {
 ...
 */
async function isEventuallyVisible(page, selector, timeout = 5000) {
  const isPresent = await isEventuallyPresent(page, selector, {
    visible: true,
    timeout
  });
  if (!isPresent) {
 ...
 */
async function isEventuallyPresent(page, selector, options = {}) {
  const defaultOptions = {
    timeout: 5000,
    logHTML: false
  };
...
function getAccountCredentials(accountName) {
  const globalConfig = _config.default.get('testAccounts');
 ...
 */
async function clickAndWaitForNewPage(page, selector, timeout = 25000) {
...
async function logHTML() {
  ...
  await (0, _slack.sendSnippetToSlack)(bodyHTML);
  return bodyHTML;
}
async function logDebugLog() {
  const log = (0, _fs.readFileSync)('/home/travis/wordpress/wp-content/debug.log').toString();
  if (log.length > 1) {
    if (process.env.E2E_DEBUG) {
      console.log('#### WP DEBUG.LOG ####');
      console.log(log);
    }
    await (0, _slack.sendSnippetToSlack)(log);
  }
}