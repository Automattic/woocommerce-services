...
async function sendFailedTestMessageToSlack(testName) {
  ...
  try {
    ...
    if (error.code === ErrorCode.PlatformError || error.code === ErrorCode.RequestError || error.code === ErrorCode.RateLimitedError || error.code === ErrorCode.HTTPError) {
  ...
  try {
    ...
    if (error.code === ErrorCode.PlatformError || error.code === ErrorCode.RequestError || error.code === ErrorCode.RateLimitedError || error.code === ErrorCode.HTTPError) {
      ...
      // Some other error, oh no!
      console.log('The error occurred does not match an error we are checking for in this block.', error);
    }
  }
}
/**
