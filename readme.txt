...
* Tweak - Update the bundled moment and lodash JavaScript libraries to versions that resolve reported security advisories (GHSA-wc69-rhjr-hc9g, GHSA-8hfj-j24r-96c4, GHSA-r5fr-rjxr-66jc).
* Tweak - Build the plugin's JavaScript on a current Node.js toolchain and drop the wp-calypso submodule from the build, with no change to the code that ships.
